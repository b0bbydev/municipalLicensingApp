...
const VehicleHistory = require("../../models/refreshmentVehicles/refreshmentVehicleHistory");
// helpers.
...
/* GET /refreshmentVehicles/vehicleHistory/:id */
router.get(
  "/:id",
  body("filterCategory")
    .matches(/^[^'";=_()*&%$#!<>\/\^\\]*$/)
  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      return res.render("refreshmentVehicles/vehicleHistory", {
        title: "BWG | Vehicle History",
        message: "Page Error!",
      ...
      if (!req.query.filterMonth && !req.query.filterYear) {
        VehicleHistory.findAndCountAll({
          where: {
        ...
        if (!req.query.filterMonth) {
          VehicleHistory.findAndCountAll({
            where: {
              // where refreshmentVehicleID = req.params.id AND year(lastModifed) = req.query.filterYear.
              [Op.and]: [
                { refreshmentVehicleID: req.params.id },
                Sequelize.where(
                  Sequelize.fn("year", Sequelize.col("lastModified")),
